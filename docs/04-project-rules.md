...
```text
понимание > скорость реализации
```

Допускается:

- переписывать код;
- намеренно сначала делать простое решение;
- моделировать ошибки;
- возвращаться к старому коду;
- менять архитектуру после появления новых требований.

Цель — понять эволюцию решения, а не сразу написать финальную архитектуру.

---

# 4. Не бежать впереди roadmap

Запрещено внедрять технологию раньше этапа, на котором она нужна, без веской причины.

Примеры:

- Kubernetes не добавляется на этапе Task CRUD;
- RabbitMQ не добавляется, если нет асинхронного межсервисного взаимодействия;
- Redis не используется «просто потому что быстрый»;
- Saga не используется для операции одного сервиса;
- Kafka не добавляется, пока RabbitMQ покрывает требования.
